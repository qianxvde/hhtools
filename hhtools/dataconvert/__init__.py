...
across ``human-humanoid-tools/scripts`` and the ``my_mjlab`` repo:
...
* :mod:`hhtools.dataconvert.isaaclab_txt` -- CSV/PKL -> isaaclab_amp
...
* :mod:`hhtools.dataconvert.profiles` -- training-export profile registry so a
  single retarget can be emitted for my_mjlab (NPZ) and isaaclab_amp
...
* :mod:`hhtools.dataconvert.fullstate` -- ``full_state`` TXT/JSON import.
...
retargeting in this package. ``my_mjlab`` consumes the NPZ and
``isaaclab_amp`` consumes the AMP TXT this package produces.
"""
